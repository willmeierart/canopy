...
export default class TransitionGroupWrapper extends Component {
	...
	} // timeout func waits for component to mount before showing
	renderComponent () {
	...
	render () {
		...
		const defaultStyle = {
			// could refactor to pass both styles and component as props
			...
			<Transition in={this.state.show} timeout={duration}>
				{state => (
					<div className='outer-wrapper' style={{ ...defaultStyle, ...transitionStyles[state] }}>
						<div className='inner-wrapper'>{this.renderComponent()}</div>
					</div>
				)}
			</Transition>
		)
